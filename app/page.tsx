...
export default function Home() {
  return (
      <div className="bg-bg h-screen md:flex">
          <Navbar/>
          {/*Page Container*/}
          <main className="flex flex-col justify-center md:flex-1 overflow-y-auto">

              {/* Hero */}
              <div
                  className="bg-white border-3 border-black p-6 w-85 md:w-[70%] lg:w-[50%] md:h-[60%] shadow-big pb-10 md:mt-10 mt-30 mx-auto md:ml-17 mb-25">
            ...
            <div className={'bg-white mt-20 border-t-6 pb-16'}>
                <h2 className={'ml-6 text-2xl font-bold py-3 underline decoration-5 underline-offset-9 decoration-main'}>MES PROJETS RECENTS</h2>

                <Link href={'/portfolio'} className={'ml-6 my-3 flex items-center gap-3 w-fit bg-main border-3 shadow-small text-white border-black px-3 py-2 '}>
                    <p>Voir plus</p>
                    <MdArrowForward />
                </Link>
            ...
            <div className={'bg-sec pb-40 border-t-6'}>
                <h2 className={'ml-6 text-2xl font-bold py-3 underline decoration-5 underline-offset-9 decoration-main'}>MES SERVICES</h2>
                <div className={'flex flex-col gap-6 mt-3'}>
                    <SkillDiv icon={<AiOutlinePicture size={70}/>}
                              title={"UI/UX DESIGN"}
                              description={"Je designe votre site web en m'adaptant à vos clients et votre business. Interface graphique, qui " +
                                  "traduit visuellement votre marque à vos clients."}/>
                    <SkillDiv icon={<PiCodeLight size={70}/>} 
                              title={"DEVELOPPEMENT"} 
                              description={"Applications et sites webs sur-mesure, rapides et sécurisés. Code maintenable et documenté."}/>
                    <SkillDiv icon={<SlSocialGoogle size={70}/>}
                              title={"Visibilité"}
                              description={"Optimisation SEO et accompagnement dans l'optimisation Google Maps, pour une experience utilisateur" +
                                  "optimale."}/>
                </div>
